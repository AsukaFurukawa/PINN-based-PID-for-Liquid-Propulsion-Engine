...
def add_combustion_visualization(fig, chamber_length, chamber_radius, perf_data):
    """Add combustion visualization inside the chamber"""
    # Create points for combustion
    num_particles = 100
    ...
    for _ in range(num_particles):
        ...
        r = chamber_radius * (0.1 + 0.8 * np.random.random())
        theta = 2 * np.pi * np.random.random()
        z = 0.05 * chamber_length + 0.85 * chamber_length * np.random.random()
        ...
        # Color based on position and chamber pressure
        # Higher intensity near the injector (z=0)
        intensity = (0.3 + 0.7 * (1 - z/chamber_length)) * (perf_data["chamber_pressure"] / 3.0)
        
        if z < 0.3 * chamber_length:  # Near injector
            # Yellow/white hot - ensure alpha is between 0.1 and 1
            alpha = max(0.1, min(0.7 * intensity, 0.99))  # Ensure alpha is valid
            colors.append(f'rgba(255, {180 + int(75*intensity)}, {50 + int(200*intensity)}, {alpha})')
        else:  # Further down chamber
            # Orange/red - ensure alpha is between 0.1 and 1
            alpha = max(0.1, min(0.5 * intensity, 0.99))  # Ensure alpha is valid
