...
def create_simple_engine_viz(height=600):
    ...
    with viz_container:
        # Create the three.js visualization using iframe HTML component
        st.markdown("### 3D Engine Visualization")
        
        # Create a simple HTML component with Three.js
        html_content = f"""
        <html>
        <head>
            <title>Rocket Engine Visualization</title>
        ...
        <body>
            <script>
                // Engine parameters from Streamlit
                const params = {json.dumps(st.session_state.engine_params)};
                ...
                // Create the engine
                const engine = createRocketEngine(params);
                scene.add(engine);
                ...
                function animate() {{
                    ...
                    }} else if (params.status === "Startup" || params.status === "Shutdown Sequence") {{
